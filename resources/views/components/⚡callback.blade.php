...
<div class="mx-auto lg:mx-0 w-full max-w-70">
    @if ($submitted)
        <div
            class="flex justify-center mx-auto lg:mx-0 items-center gap-3 bg-emerald-800 border border-emerald-500/40 backdrop-blur-md rounded-2xl px-5 py-3.5 text-emerald-300 w-full text-center text-sm font-semibold animate-in fade-in duration-300">
    ...
    @else
        <form wire:submit="save" class="flex flex-col gap-1 mx-auto lg:mx-0 w-full">
            <div
                class="flex justify-between items-center gap-3 bg-emerald-800 border border-white/15 rounded-2xl px-5 py-2.5 transition-all focus-within:bg-black/40 focus-within:border-white/40 w-full">
                {{-- Поле введення --}}
