...
<div class="relative h-72 md:h-80 bg-cover bg-center" style="background-image: url('{{ $image }}')">
    ...
    <div class="relative flex flex-col items-center justify-end size-full px-5 pb-10 md:pb-14">
        <h1
            {{ $title->attributes->class('font-display text-2xl md:text-4xl text-white text-center text-balance font-bold leading-tight') }}>
            {{ $title }}
        </h1>
        @isset($description)
            <p
                {{ $title->attributes->class('text-sm md:text-base text-white/70 text-center text-balance max-w-lg mt-3 leading-relaxed mb-0') }}>
                {{ $description }}
            </p>
        @endisset
    </div>
