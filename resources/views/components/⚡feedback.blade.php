...
<div x-data="{
    ...
    @else
        <form @submit.prevent="submit()" class="relative space-y-5">
            <div>
                <x-forms.input size="lg" wire:model="feedback.name" icon="user"
                    placeholder="Ваше ім'я (як до вас звертатися?)" />
            </div>

            <div>
                <x-forms.input size="lg" wire:model="feedback.contact" icon="mail"
                    placeholder="Ваші контакти (email або телефон)" />
            </div>

            <div>
                <x-forms.select required wire:model.live="feedback.topic" icon="help-circle" placeholder="Тема звернення"
                    size="lg">
                ...
                @error('feedback.topic')
                    <x-forms.error class="ml-1 mt-2" :message="$message" />
                @enderror
            </div>
            ...
            <div>
                <x-forms.select wire:model="feedback.service" icon="sparkles" placeholder="Послуга (опціонально)"
                    size="lg">
